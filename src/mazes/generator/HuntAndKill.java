...
public class HuntAndKill extends MazeGenerator{
    ...
    public Grid generate(int rows, int columns) {
        ...
        GridCell v = g.getCellByCoordinates(rows - 1, columns - 1);

        while (v != null) {
            v.visited = true;
            // ottenimento delle celle adiacenti a v non visitate
            ArrayList<GridCell> unvisitedAdjacent = g.getCellNearTo(v);
    ...
    protected GridCell findNextStart(Grid g, int rows, int columns) {
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                GridCell a = g.getCellByCoordinates(i, j);
                ArrayList<GridCell> visitedAdjacent = g.getCellNearTo(a);
